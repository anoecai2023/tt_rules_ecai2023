...
for seed in [0,1,6,7,8]:
    X_df, y_df, f_df, label_pos = read_csv("../dataset" + "/diabetes/diabetes" + ".data",
                                           "../dataset" + "/diabetes/diabetes" + ".info",
                                           shuffle=True)
    db_enc = DBEncoder(f_df)
    db_enc.fit(X_df, y_df)
    _, y = db_enc.transform(X_df, y_df, normalized=True, keep_stat=True)
    y = np.argmax(y, axis=1)

    from aix360.algorithms.rbm import FeatureBinarizer
    fb = FeatureBinarizer(negations=True)
    X = fb.fit_transform(pd.DataFrame(X_df))
    ...
    # X_train_fb = pd.DataFrame(X_train)
    # X_test_fb = pd.DataFrame(X_test)
    ...
    #DT
    #0.572211850250565 +/- 0.0023282622729087032
    #LR
    # 0.5811241033703449 +/- 0.0025058233904710686
    #RF
    #0.5915466247420654 +/- 0.0013736843562779802
    #DNN
    # 0.64 +/-
