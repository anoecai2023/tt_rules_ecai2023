...
truth_tables = np.array(truth_tables).transpose()

n_filters = max(n_filters) + 1

with open(os.path.join(path_save_model, 'human_expressions', 'with_dontcare.txt'), 'r') as f:
    cpt_gates = 0
    for line in f.readlines():
        if "Class 1" in line:
            # same expressions for all class with different weights
            break
        cpt_gates += str(line).count("&")
        cpt_gates += str(line).count("|")

print('Number of total gates with dont care: ', cpt_gates)

with open(os.path.join(path_save_model, 'human_expressions', 'positive_correlation.txt'), 'r') as f:
    pos_corr = ast.literal_eval(f.read())

with open(os.path.join(path_save_model, 'human_expressions', 'negative_correlation.txt'), 'r') as f:
    neg_corr = ast.literal_eval(f.read())
...
for filter_idx1, filter_idx2, position in pos_corr:
    # print(filter_idx1*neurons_per_filter + position, filter_idx, pos1, pos2)
    W[:, filter_idx1 * neurons_per_filter + position] = W[:, filter_idx1 * neurons_per_filter + position] + W[:,
                                                                                                            filter_idx2 * neurons_per_filter + position]
    W[:, filter_idx2 * neurons_per_filter + position] = 0
    # truth_tables[:, filter_idx1 * neurons_per_filter + position] = truth_tables[:,
    #                                                                filter_idx2 * neurons_per_filter + position].copy()
for filter_idx1, filter_idx2, position in neg_corr:
    # print(filter_idx1*neurons_per_filter + position, filter_idx, pos1, pos2)
    W[:, filter_idx1 * neurons_per_filter + position] = W[:, filter_idx1 * neurons_per_filter + position]-W[:, filter_idx2 * neurons_per_filter + position]
    W[:, filter_idx2 * neurons_per_filter + position] = 0
    # truth_tables[:, filter_idx1 * neurons_per_filter + position] = -truth_tables[:,
    #                                                                 filter_idx2 * neurons_per_filter + position].copy()
...
with open(os.path.join(path_save_model, 'human_expressions', 'with_dontcare.txt'), 'r') as f:
    ...
    for line in f.readlines():
        ...
        try:
            ...
            continue
        position = i % neurons_per_filter
        filter_idx = i // n_filters
        for filter_idx2 in range(n_filters):
            if (filter_idx, filter_idx2, position) in pos_corr or (filter_idx, filter_idx2, position) in neg_corr:
                flag_count = False
                break
...
print('Number of total gates with dont care and correlation: ', cpt_gates)
...
with torch.no_grad():
    for i, vdata in tqdm(enumerate(train_loader)):
        ...
        filter1 = [[], [], [], []]
        for pixel in range(truth_tables.shape[1]):
            block = truth_tables[:, pixel]
            patch = filter_input[:, pixel % neurons_per_filter, :]
            patch = BitsToIntAFast(patch).astype(int)
            out_block = np.expand_dims(block[patch], axis=-1)
            all_filters.append(out_block)

        in_classifier = np.concatenate(all_filters, axis=-1)
        out = in_classifier.copy()

        if thrs['poly_act']:
            out = out @ w1.transpose() + b1
            out = alpha + beta * out + gamma * out ** 2
            out = out @ w2.transpose() + b2
            out = softmax(out)
        else:
            out = out @ W.transpose() + B

        if pred_proba is None:
            pred_proba = out[:, -1]
        else:
            pred_proba = np.concatenate((pred_proba, out[:, -1]), axis=0)

        if args.dataset in ['heloc', 'house']:
            losstot += lossmse(vlabels, torch.Tensor(out)[:, -1])

        vpred = np.argmax(out, axis=1)
