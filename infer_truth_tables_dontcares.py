...
truth_tables_nodc = np.load(os.path.join(path_save_model, "TruthTable.npy"))
...
with torch.no_grad():
    for i, vdata in tqdm(enumerate(train_loader)):
        ...
        all_filters_no_dc = []
        for filter_idx in range(truth_tables_nodc.shape[1]):
            block = truth_tables_nodc[:, filter_idx]
            filter_input = inpt.squeeze()[:, indexes]
            filter_input = BitsToIntAFast(filter_input).astype(int)
            all_filters_no_dc.append(np.expand_dims(block[filter_input], axis=-1))

        in_classifier_no_dc = np.concatenate(all_filters_no_dc, axis=-1).transpose(0, 2, 1)
        out_no_dc = in_classifier_no_dc.reshape((in_classifier_no_dc.shape[0], W.shape[1]))

        assert np.array_equal(out, out_no_dc)
